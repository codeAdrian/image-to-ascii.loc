<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><title>Image To ASCII | By: Adrian Bece</title><link href="css/styles.css" rel="stylesheet"></head><header class=container><h1><span class=large-logo aria-hidden=true>██  ██      ██  ███████  ███████  ██████    ██████  ██████        ███████  ██████  ██████  ██  ██<br>██  ████  ████  ██   ██  ██       ██          ██    ██  ██        ██   ██  ██   █  ██  ██  ██  ██<br>██  ██  ██  ██  ███████  ██ ████  ████  ████  ██    ██  ██  ████  ███████    ██    ██      ██  ██<br>██  ██      ██  ██   ██  ██   ██  ██          ██    ██  ██        ██   ██  █   ██  ██  ██  ██  ██<br>██  ██      ██  ██   ██  ███████  ██████      ██    ██████        ██   ██  ██████  ██████  ██  ██<br></span><span class=screen-reader-content>Image To ASCII</span></h1></header><section class=container><article class='<?php if($_SERVER["REQUEST_METHOD"] == "POST") echo "hidden"; ?>'><p>This simple website allows users to upload an image from their computer and convert them to ASCII art. Selected JPEG image will be temporarly moved to "uploads" folder during the conversion process and is deleted after the process has been finished. PHP is used to analyse the image, convert it from RGB to Grayscale using Luma conversion algorithm and output the ASCII character for each pixel. For more information about ASCII art, check out <a href=https://en.wikipedia.org/wiki/ASCII_art title="Wikiepdia - ASCII art">this article on Wikipedia</a>.</p></article><article class="box-input"><form enctype="multipart/form-data" method="post" action='<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>' onsubmit="loadingMessage()">
            <?php
            function grayscale($rgb)
            {
                // Convert RGB pixel to Grayscale using LUMA algorithm
                return round(0.3 * $rgb["red"] + 0.59 * $rgb["green"] + 0.11 * $rgb["blue"]);
            }
            function getIndex($gray)
            {
                // Returns index 0-18 in character map
                return intval($gray/14);
            }
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (!empty($_FILES['file'])) {
                    ...
                    // Check of it is a jpg image
                    $allowed =  array('jpeg','jpg');
                    if (!in_array(pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION),$allowed)) echo "<div class=\"box-error\">An error has occured. Selected file is not a jpg image.</div>";
                    // Temporarly upload a file to the server
                        ...
                        if($size_h <= 1024 && $size_w <= 1024){
                            echo '<article class="box-output"><div><div><p>Converted File: '.$_FILES['file']['name'].'</p></div><label>Adjust Preview Scale: </label><input id="scaleImage" type="range" value="10" min="2" max="24" step="1" onchange="printValue(\'scaleImage\',\'asciiOutput\');" /></div>';
                    ...
                    // Fail gracefully
                    else echo "<div class=\"box-error\">An error has occured. Please refresh and try again.</div>";
                }
            }
            ?>
            ...
            <ul class="combined-container">
                ...
                <li>Upload the selected image and let the magic happen
                    <button type="submit" onclick="clearContents()">Convert Image</button>
                </li>
